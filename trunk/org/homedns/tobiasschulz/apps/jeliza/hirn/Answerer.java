...
public class Answerer {
	 ...
	 * @return Die Antwort
	 */
	public String parseSatzOld(String fraParameter, String antParameter) {
		String ant = antParameter;
		String fra = fraParameter;
		...
		if (spm.satzType == SatzParseManager.ERWEITERTE_FRAGE) {
			if (fragewort.trim().toLowerCase().hashCode() == "was".hashCode()) {
				try {
					ant = FileManager.readFileIntoString(
			...
			if (fragewort.trim().toLowerCase().hashCode() == "wer".hashCode()) {
				try {
					ant = FileManager.readFileIntoString(
							absoluteUrl + "wortschatz/ext-ques/wer/" + subjekt
									+ "/" + verb + "/null").trim();
				} catch (IOException e) {
					return ant;
				}
			...
			if (fragewort.trim().toLowerCase().hashCode() == "wie".hashCode()) {
				try {
					ant = FileManager.readFileIntoString(
							absoluteUrl + "wortschatz/ext-ques/wie/" + subjekt
									+ "/" + verb + "/null").trim();
				} catch (IOException e) {
					return ant;
				}
			...
			if (fragewort.trim().toLowerCase().hashCode() == "wo".hashCode()) {
				try {
					ant = FileManager.readFileIntoString(
							absoluteUrl + "wortschatz/ext-ques/wo/" + subjekt
									+ "/" + verb + "/null").trim();
				} catch (IOException e) {
					return ant;
				}
	...
	public String parseSatz(String fraParameter, String antParameter) {
		...
		if (spm.satzType == SatzParseManager.EINFACHE_FRAGE) {
			...
			try {
				in.useInterpreter("simple-ques.jel");
			...
			if (ans.othr.hashCode() == "true".hashCode()) {
				return "Ja";
			}
			if (ans.othr.hashCode() == "false".hashCode()) {
				return "Nein!";
			}
		...
		if (spm.satzType == SatzParseManager.ERWEITERTE_FRAGE) {
			try {
				in.useInterpreter("ext-ques.jel");
			} catch (FileNotFoundException e) {
				e.printStackTrace();
			}
