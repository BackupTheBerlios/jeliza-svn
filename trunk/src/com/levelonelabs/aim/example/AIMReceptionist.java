...
public class AIMReceptionist {
	...
	public static void main(String[] args) {
		...
		// buddy list
		final AIMSender aim = new AIMClient(username, pass,
				"AIM Receiptionist", true);
		...
		aim.addAIMListener(new AIMAdapter() {
			...
			public void handleMessage(AIMBuddy buddy, String request) {
				...
				if (newName.isOnline()) {
					aim.sendMessage(newName, buddy.getName() + " said: "
							+ request);
				} else {
					newName.addMessage(buddy.getName() + " said \"" + request
							+ "\" at " + new Date());
				}
			}
			...
			public void handleBuddySignOn(AIMBuddy buddy, String info) {
				if (buddy.getName().equalsIgnoreCase(newName.getName())
						&& buddy.hasMessages()) {
