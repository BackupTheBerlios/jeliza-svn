...
public class ScheduleModule extends BotModule implements XMLizable {
	...
	private static Logger logger = Logger.getLogger(ScheduleModule.class
			.getName());
	...
	private class SchedulerTask extends TimerTask {
		...
		public void run() {
			System.err.println("Timeout for: " + buddy.getName() + " cmd:"
					+ cmd);
	...
	public String help() {
		...
		sb
				.append("<B>schedule <i>TEXT</i></B> (stores text in the system for retrieval later)\n");
		sb
				.append("<B>cancel <i>All or #</i></B> (removes all items or a specific item)\n");
		...
		if (br != null) {
			...
			if (br.size() != 0) {
				...
				for (int i = 0; i < br.size(); i++) {
					...
					eventlist += ((i + 1) + ")" + (st.getCommand()
							+ " scheduled " + st.getCreateDate() + "<BR>"));
				}
			} else
	...
	private void handleSchedule(AIMBuddy buddy, String query) {
		...
		try {
			...
			while (st.hasMoreTokens() && stop == 0) {
				token = st.nextToken();
				System.err.println("token: " + token + "state:"
				...
				case 2:
					...
					if (at_idx != -1 && delim_idx != -1) {
								...
								.println("Schedtoken:"
										+ query
												.substring(at_idx + 2,
														delim_idx).trim());
						try {
							eventDate = sdf.parse(query.substring(at_idx + 2,
									delim_idx).trim());
							state = 51;
						} catch (Exception e) {
							System.err
									.println("Can't parse date & time, trying just time..");
							state = 99;
						}
						...
						try {
							...
							System.err.println("event date:"
									+ eventDate.toString());
							System.err.println("today's date:"
									+ todaysDate.toString());
							...
							eventDate.setTime(eventDate.getTime()
									+ todaysDate.getTime() - 18000000);
							state = 51;
						} catch (Exception e) {
							System.err
									.println("Can't parse date or time, bailing");
							state = 99;
						}
				...
				case 3:
					...
					period = Long.parseLong(token.substring(0,
							token.length() - 1));
					state = 50;
				...
				case 50:
					...
					if (token.equals("-")) {
						stop = 1;
						command = query.substring(query.indexOf('-') + 1)
								.trim();
					} else
						state = 99;
					break;
			...
			// list the events
			{
				...
				// "IN"
				// type
				System.err.println("Scheduling " + command + " in "
				...
				// "AT"
				// type
				System.err.println("Scheduling " + command + " at "
						+ eventDate.toString());
				...
				sendMessage(buddy,
						"That schedule command is not implemented yet");
		} catch (Exception e) {
			sendMessage(buddy, "Unable to understand schedule command");
			e.printStackTrace();
		}
		...
		while (iter.hasNext()) {
			...
			if (br.size() > 0) {
				...
				for (int i = 0; i < br.size(); i++) {
					...
					CDATASection buddy_data = doc.createCDATASection(st
							.getBuddy().getName());
					buddy.appendChild(buddy_data);

					Element cmd = doc.createElement("command");
					CDATASection cmd_data = doc.createCDATASection(st
							.getCommand());
					cmd.appendChild(cmd_data);

					Element createDate = doc.createElement("createDate");
					CDATASection cd_data = doc.createCDATASection(st
							.getCreateDate());
