 ...
 * 
 * @todo instead of string, store HistoryObject (signon/off, date), then can
 *       easily compute on/off line durations
...
public class UserInformationModule extends BotModule {
	...
	public String help() {
		...
		sb
				.append("<B>history <i>USER</i></B> (displays user's recent sign on and off history)\n");
		sb.append("<B>users</B> (displays status of all users)\n");
		sb
				.append("A="
						+ AIMBot.ROLE_ADMINISTRATOR
						+ ", M = messages pending, * = new user, E = Enemy, - = banned\n");
	...
	public void performService(AIMBuddy buddy, String query) {
		if (query.toLowerCase().startsWith("history")) {
			...
			ArrayList hist = (ArrayList) userHash.get(to.getName()
					.toLowerCase());
			if (hist == null) {
				sendMessage(buddy, "Sorry, I have no history for user: "
						+ to.getName());
				return;
			} else {
				String result = "";
			...
			for (Iterator iter = getBuddyNames(); iter.hasNext();) {
				...
				result += (bud.getName() + "["
						+ (bud.isOnline() ? "<B>ON</B>" : "<I>OFF</I>") + "]"
						+ markup + "    ");
			}
			if (result.trim().equals("")) {
				sendMessage(buddy,
						"Sorry, I have no information about active users.");
				return;
			} else {
			...
			if (result.trim().equals("")) {
				...
				sendMessage(buddy, "Current (<b>" + num
						+ "</b>) Enemies are:\n" + result);
				return;
			}
		}
