...
public class HeadlinesModule extends BotModule {
	...
	static {
		...
		channelName = "zdnet";
		channel = new Channel(channelName,
				"http://p.moreover.com/cgi-local/page?feed=129&o=rss");
		services.add(channelName);
		channels.put(channelName.toLowerCase(), channel);
		...
		channelName = "fark";
		channel = new Channel(channelName,
				"http://www.newsisfree.com/HPE/xml/feeds/17/1717.xml");
		services.add(channelName);
		channels.put(channelName.toLowerCase(), channel);

		channelName = "jworld";
		channel = new Channel(channelName,
				"http://www.newsisfree.com/HPE/xml/feeds/59/959.xml");
		services.add(channelName);
		channels.put(channelName.toLowerCase(), channel);

		channelName = "mets";
		channel = new Channel(channelName,
				"http://p.moreover.com/cgi-local/page?feed=559&o=rss");
		services.add(channelName);
		channels.put(channelName.toLowerCase(), channel);

		channelName = "islanders";
		channel = new Channel(channelName,
				"http://p.moreover.com/cgi-local/page?feed=336&o=rss");
		services.add(channelName);
		channels.put(channelName.toLowerCase(), channel);

		channelName = "nfl";
		channel = new Channel(channelName,
				"http://www.newsisfree.com/HPE/xml/feeds/52/1852.xml");
		services.add(channelName);
		channels.put(channelName.toLowerCase(), channel);

		channelName = "deals";
		channel = new Channel(channelName,
				"http://www.totaldeals.com/rss/new.asp");
		services.add(channelName);
		channels.put(channelName.toLowerCase(), channel);

		channelName = "cnet";
		channel = new Channel(channelName,
				"http://www.moreover.com/cgi-local/page?feed=139&o=rss");
		services.add(channelName);
		channels.put(channelName.toLowerCase(), channel);
	}
	...
	public String help() {
		...
		sb
				.append("Type one of the following site names to see the latest headlines:\n");
		...
		if (chan != null) {
			...
			if (arr.size() > 0) {
				for (int i = 0; (i < arr.size())
						&& (i < HeadlinesModule.MAX_NUM_HEADLINES); i++) {
