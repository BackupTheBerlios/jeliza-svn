...
public class ListModule extends BotModule {
	...
	public String help() {
		...
		sb
				.append("<B>subscribe <i>LISTNAME</i> </B> (subscribe yourself to the named list.)\n");
		sb
				.append("<B>unsubscribe <i>LISTNAME</i> </B> (unsubscribe yourself from the named list.)\n");
		sb
				.append("<B>post <i>LISTNAME</i> <i>MESSAGE</i></B> (will post the message to everyone in the list."
						+ "  Offline members will be told when they sign on.)\n");
		sb
				.append("<B>members <i>LISTNAME</i></B> (will show the members of everyone in the list. *ADMIN ONLY*)\n");
		sb
				.append("<B>makelist <i>LISTNAME</i></B> (creates a new list. *ADMIN ONLY*)\n");
		sb
				.append("<B>removelist <i>LISTNAME</i></B> (Removes a list *ADMIN ONLY*)\n");
		sb
				.append("<B>addlistmember <i>LISTNAME BUDDYNAME</i></B> (adds a user to a list) *ADMIN ONLY*)\n");
		sb
				.append("<B>rmlistmember <i>LISTNAME BUDDYNAME</i> </B> (removes a user from a list) *ADMIN ONLY*)\n");
		return sb.toString();
	}
	...
	private void processLists(AIMBuddy buddy) {
		...
		for (Enumeration enumer = super.bot.getGroups(); enumer
				.hasMoreElements();) {
			AIMGroup next = (AIMGroup) enumer.nextElement();
			lists.append(next.getName() + " with " + next.size()
					+ " members.\n");
		}
		...
		if (!buddy.hasRole(AIMBot.ROLE_ADMINISTRATOR)) {
			...
			if (group != null) {
				if (group.remove(buddyname)) {
					super.sendMessage(buddy, buddyname
							+ " was successfully removed from list:" + list);
				} else {
					super.sendMessage(buddy, buddyname
							+ " was not a member of list:" + list);
				}
			} else {
				super.sendMessage(buddy, "The list, " + list
						+ ", does not exist.");
			}
	...
	private void processAddListMember(AIMBuddy buddy, StringTokenizer st) {
		if (!buddy.hasRole(AIMBot.ROLE_ADMINISTRATOR)) {
			...
			if (ibuddy == null) {
				super.sendMessage(buddy, buddyname
						+ " is not a user, please ADDUSER");
			} else {
				AIMGroup group = super.bot.getGroup(list);
				if (group != null) {
					if (group.add(ibuddy.getName())) {
						super.sendMessage(buddy, buddyname
								+ " was successfully added to list:" + list);
					} else {
						super.sendMessage(buddy, buddyname
								+ " was already a member of list:" + list);
					}
				} else {
					super.sendMessage(buddy, "The list, " + list
							+ ", does not exist.");
	...
	private void processRemoveList(AIMBuddy buddy, StringTokenizer st) {
		if (!buddy.hasRole(AIMBot.ROLE_ADMINISTRATOR)) {
			...
			if (group != null) {
				...
				super
						.sendMessage(buddy, "The list " + list
								+ " does not exist");
			}
		}
	}
	...
	private void processMembers(AIMBuddy buddy, StringTokenizer st) {
		if (!buddy.hasRole(AIMBot.ROLE_ADMINISTRATOR)) {
			...
			AIMGroup group = super.bot.getGroup(list);
			if (group != null) {
				super.sendMessage(buddy, "The list, " + list + ", contains: "
						+ group.toString());
			} else {
				super.sendMessage(buddy, "The list, " + list
						+ ", does not exist.");
		...
		if (group != null) {
			if (group.remove(buddy.getName())) {
				super.sendMessage(buddy,
						"You were successfully removed from list:" + list);
			} else {
				super.sendMessage(buddy, "You were not a member of list:"
						+ list);
			}
		} else {
	...
	private void processSubscribe(AIMBuddy buddy, StringTokenizer st) {
		...
		if (group != null) {
			if (group.add(buddy.getName())) {
				super.sendMessage(buddy, "You were successfully added to list:"
						+ list);
			} else {
				super.sendMessage(buddy, "You were already a member of list:"
						+ list);
			}
		} else {
			super.sendMessage(buddy, "The list, " + list + ", does not exist.");
	...
	private void sendGroupMessage(AIMGroup group, String message, AIMBuddy from) {
		...
		for (ListIterator it = group.getList().listIterator(); it.hasNext();) {
			...
			if (next.isOnline()) {
				super.sendMessage(next, from.getName() + " posted to the "
						+ group.getName() + " list:" + message);
				toldNow++;
			} else {
				next.addMessage(from.getName() + " posted to the "
						+ group.getName() + " list: \"" + message + "\" at "
						+ new Date());
			}
		}
		super.sendMessage(from, "Told " + toldNow + " of " + group.size()
				+ " list members.");
	}
}
