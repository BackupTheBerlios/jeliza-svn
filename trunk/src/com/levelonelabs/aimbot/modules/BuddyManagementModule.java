...
public class BuddyManagementModule extends BotModule {
	...
	static Logger logger = Logger.getLogger(BuddyManagementModule.class
			.getName());
	...
	public String help() {
		...
		sb
				.append("<B>makeusers</B> (converts all non-user buddies to users *ADMIN ONLY*)\n");
		sb
				.append("<B>prune buddies</B> (removes all non-user buddies *ADMIN ONLY*)\n");
		sb
				.append("<B>prune inactive</B> (removes \"inactive\" users *ADMIN ONLY*)\n");
		return sb.toString();
	}
	...
	public void performService(AIMBuddy buddy, String query) {
		...
		if (imcommand.equalsIgnoreCase("makeusers")) {
			...
			while (iter.hasNext()) {
				...
				if (!bud.hasRole(AIMBot.ROLE_USER)
						&& !super.bot.getUsername().equals(name)) {
					logger.fine("Adding " + AIMBot.ROLE_USER + " role to "
							+ bud.getName());
					bud.addRole(AIMBot.ROLE_USER);
					numConverted++;
				}
			}
			super.sendMessage(buddy, "Converted " + numConverted
					+ " buddies to users.");
			return;
			...
			if (option.equalsIgnoreCase("buddies")) {
				...
				while (iter.hasNext()) {
					...
					// us
					if (!bud.hasRole(AIMBot.ROLE_USER)
							&& !bud.hasRole(AIMBot.ROLE_ADMINISTRATOR)
				...
				super.sendMessage(buddy, "Removed " + numRemoved
						+ " non-user buddies.");
				return;
				...
				while (iter.hasNext()) {
					...
					if (bud.hasRole(AIMBot.ROLE_USER)
						...
						if (isInactive(bud)) {
							logger.fine("Removing inactive buddy "
									+ bud.getName());
							toRemoveList.add(bud);
							numRemoved++;
						}
