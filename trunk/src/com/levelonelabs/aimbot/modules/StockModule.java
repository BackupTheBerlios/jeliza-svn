...
public class StockModule extends BotModule {
	...
	/**
	 * http://finance.yahoo.com/d/quotes.csv?f=snl1d1t1c1ohgv&s=GE (s= is the
	 * symbol to get, f= is the order of the info) Here is list of known
	 * variables that can be passed as options for "f=" s = Symbol l9 = Last
	 * Trade d1 = Date of Last Trade t1 = Time of Last Trade c1 = Change c =
	 * Change - Percent Change o = Open Trade h = High Trade g = Low Trade v =
	 * Volume a = Ask Price b = Bid Price j = 52 week low k = 52 week high n =
	 * Name of company p = Previous close x = Name of Stock Exchange
	 */
	...
	public String help() {
		...
		sb
				.append("<B>stock <i>SYMBOLS</i></B> (displays stock price and other info for the given space separated symbols)\n");
		sb
				.append("* If the preference \"portfolio\" is set, you can omit the symbols to use those listed in the preference.");
	...
	public void performService(AIMBuddy buddy, String query) {
		if (query.trim().toLowerCase().startsWith("stock")) {
			...
			if (st.countTokens() < 2 && portfolioPref == null) {
				...
				// grab the quote
				try {
					String queryURL = URL + "?" + "f=" + this.options + "&s="
							+ symbol;
