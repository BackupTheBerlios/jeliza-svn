...
public class MessengerModule extends BotModule {
	...
	private static Logger logger = Logger.getLogger(MessengerModule.class
			.getName());
	...
	public String help() {
		...
		sb
				.append("<B>tell <i>BUDDY MESSAGE</i></B> (sends a message to a person in ");
		sb
				.append("the system, if they are offline they will recieve the message ");
		sb.append("next time they log in)\n");
		sb
				.append("<B>offline <i>BUDDY MESSAGE</i></B> (will notify the person the next time they log in)\n");
		sb
				.append("<B>clear messages</B> (will erase messages in system for user)\n");
		sb
				.append("<B>show messages</B> (will display all the messages left for a user)\n");
		sb
				.append("* If the preference \"savemessages\" is set to true, need to manually \"clear messages\".\n");
		...
		if (query.toLowerCase().startsWith("tell")) {
			...
			if (!imcommandText.equals("")) {
				...
				if (to.isOnline()) {
					...
					sendMessage(buddy, imcommandTo
							+ " is offline and will be told when they signon.");
	...
	private void handleOffline(AIMBuddy buddy, String query) {
		...
		if (query.toLowerCase().startsWith("offline")) {
			...
			if (!imcommandText.equals("")) {
				sendMessage(buddy, imcommandTo
						+ " will be told next time they signon.");
				addMessage(to, buddy, imcommandText);
			}
		}
	}
