...
public class TelnetModule extends BotModule {
	...
	public String help() {
		...
		sb
				.append("The Telnet module does not support commands through this interface.\n");
		return sb.toString();
	}
	...
	public class TelnetServer extends Thread {
		...
		public TelnetServer(AIMBot bot) {
			try {
				...
				logger
						.severe("Could not create server socket in Telent Module");
				bRunSvr = false;
			}
		}
		...
		public class TelnetHandler extends Thread {
			...
			public void run() {
				...
				try {
					osw
							.write("Welcome to Jaimbot's Telnet Module.  Type quit to quit.\n");
					osw.flush();
					do {
						while ((ch = isr.read()) != '\n') {
