...
public class TVListingsModule extends BotModule {
	...
	public String help() {
		...
		sb
				.append("<B>tvlist <i>ZIPCODE</i></B> (displays the tv providers in the area of the zipcode)\n");
		sb
	...
	public void performService(AIMBuddy buddy, String query) {
		if (query.trim().toLowerCase().startsWith("tvlist")) {
			...
			if ((zipPref == null) && (st.countTokens() < 2)) {
				...
				if (zipcode.length() == 5) {
					...
					if (providers.size() > 0) {
						...
						for (Iterator iter = providers.keySet().iterator(); iter
								.hasNext();) {
			...
			if ((provider == null) || provider.trim().equals("")) {
				...
				TVListingsScraper listings = new TVListingsScraper(
						searchString, provider);
				ShowDetail showDetail = listings.getShowDetail();

				String result = "Unable to find show information for "
						+ searchString;

				if ((showDetail != null) && (showDetail.getUrl() != null)
						&& (showDetail.getShow() != null)) {
					TVResultsScraper scrapper = new TVResultsScraper(showDetail
							.getUrl(), showDetail.getShow());
					...
					if (summary.trim().equals("")) {
						result = "<a href=\"" + showDetail.getUrl() + "\">"
								+ showDetail.getShow() + "</a>";
					} else {
...
class TVProviderScraper extends ParserCallback {
	...
	public void handleStartTag(HTML.Tag t, MutableAttributeSet a, int pos) {
		if ((t == HTML.Tag.SELECT)
			...
			validSelect = true;
		} else if ((t == HTML.Tag.OPTION) && validSelect
				&& (a.getAttribute(HTML.Attribute.VALUE) != null)) {
	...
	public Map getProviders() {
		...
		try {
			...
			logger.fine("Looking for prividers for:" + zipcode + " using URL= "
					+ url);
...
class TVListingsScraper extends ParserCallback {
	...
	public void handleStartTag(HTML.Tag t, MutableAttributeSet a, int pos) {
		if (!foundURL) {
			if (grabURL) {
				try {
					String urlString = a.getAttribute(HTML.Attribute.HREF)
							.toString();
	...
	public ShowDetail getShowDetail() {
		try {
			URL url = new URL(BASE_URL + SEARCH_URL + SHOW_URL + this.show
					+ PROVIDER_URL + this.provider + END_URL);
			...
			if (logger.isLoggable(Level.FINEST)) {
				...
				for (Iterator iter = headers.keySet().iterator(); iter
						.hasNext();) {
					String key = (String) iter.next();
