...
public class SkiReportModule extends BotModule {
	...
	public String help() {
		...
		sb
				.append("<B>ski <i>STATE ABBREVIATION:RESORT CODE</i></B> (displays the ski reports for the given resort)\n");
		sb
	...
	public void performService(AIMBuddy buddy, String query) {
		if (query.trim().toLowerCase().startsWith("snow")
				|| query.trim().toLowerCase().startsWith("ski")) {
			...
			if ((mntPref == null) && (st.countTokens() < 2)) {
				...
				if (ind <= 0) {
					super.sendMessage(buddy,
							"ERROR: must use [STATE ABBREVIATION:RESORT CODE] format (e.g. CO:77)\n"
									+ help());
					return;
				} else {
				...
				if (!resortCode.trim().equals("") && !state.trim().equals("")) {
					...
					if (temp != null) {
						result = temp.getTitle() + "\n" + "<b>New Snow:</b> "
								+ temp.getNewSnow() + "  \t"
								+ "<b>Surface:</b> " + temp.getSurfaceSnow()
								+ "  \t" + "<b>Base:</b> "
								+ temp.getBaseDepth() + "\n"
								+ "<b>Open Runs:</b> " + temp.getRunsOpen()
								+ "  \t" + "<b>Open Lifts:</b> "
								+ temp.getLiftsOpen() + "  \t"
								+ "<b>Open Acres:</b> " + temp.getAcresOpen();

						if (mntPref == null) {
							// if the buddy didnt have a mountain perference
							// set, set this one for him
							buddy.setPreference(MOUNTAIN_PREF, state + ":"
									+ resortCode);
						}
					}
				}
			...
			if (st.countTokens() == 2) {
				...
				if (state.length() == 2) {
					...
					if (mntMap.size() > 0) {
						...
						for (Iterator iter = mntMap.keySet().iterator(); iter
								.hasNext();) {
							String key = (String) iter.next();
							String value = (String) mntMap.get(key);
							logger.fine(value + "\t use --> " + state + ":"
									+ key);
...
class MountainScraper extends ParserCallback {
	...
	public void handleStartTag(HTML.Tag t, MutableAttributeSet a, int pos) {
		if (t == HTML.Tag.A && validLink) {
			...
			if (ind >= 0) {
				try {
					String temp = urlString.substring(ind
							+ (BASE_URL + this.state + "/").length());
					...
					logger.fine("Parser didn't handle parsing [" + urlString
							+ "] cleanly:" + e.getMessage());
				}
			}
		}
	}
	...
	public Map getMountains() {
		...
		try {
			...
			logger.fine("Looking for mountains in:" + state + " using URL= "
					+ url);
			...
			logger.fine("Error looking for mountains, ERROR:"
					+ e.getStackTrace());
			return new HashMap();
		}
		return result;
...
class SkiScraper extends ParserCallback {
	...
	public SnowReport getSnowReport() {
		...
		logger.fine("Looking for Snow report for resort " + resortCode
				+ " in state" + state + " using URL= " + url);
...
class SnowReport {
	...
	public String toString() {
		return title + ":\n" + "New Snow: " + newSnow + "\t\t" + "Surface: "
				+ surfaceSnow + "\t\t" + "Base: " + baseDepth + "\n"
				+ "Open Runs: " + runsOpen + "\t\t" + "Open Lifts: "
				+ liftsOpen + "\t\t" + "Open Acres: " + acresOpen;
	}
}