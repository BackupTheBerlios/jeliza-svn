...
public class BabelModule extends BotModule {
	...
	private static Logger logger = Logger
			.getLogger(BabelModule.class.getName());
	...
	private static String runQuery(String passage, String translation) {
		try {
			...
			String query = base + "?" + "doit=done" + "&intl=1" + "&tt=urltext"
					+ "&urltext=" + URLEncoder.encode(marker + " " + passage)
					+ "&lp=" + translation;
	...
	private static String makePoetry(String input, int numtrials) {
		...
		for (int i = 0, actual_count = 0; (i < numtrials)
				&& (actual_count <= MAX_TRIALS); i++, actual_count++) {
	...
	public String help() {
		StringBuffer sb = new StringBuffer();
		sb
				.append("<B>babel [-n <i>DISTORTION</i>] <i>TEXT</i></B> (creates poetry from <TEXT>, this may take several seconds)\n");
		sb
				.append("NOTE: Using -n optionally specifies amount to distort meaning (MAX:"
						+ MAX_TRIALS + ")");
		sb
