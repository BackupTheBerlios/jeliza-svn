...
/**
 * Handles requests to get movie times based on zip and/or theater and lists
 * theaters by zip code.
 * 
...
public class MoviesModule extends BotModule {
	...
	public String help() {
		...
		sb
				.append("<B>theaters <i>ZIPCODE<i></B> (displays a list of theaters in your area)\n");
		sb
				.append("* If the preference \"zipcode\" is set, you can omit the zipcode to use your default.\n");
		sb
				.append("<B>movies <i>ZIPCODE [Theater Number]</i></B> (displays movie times for the theater closest to your zipcode if a theater number is not provided)\n");
		sb
				.append("* If the preference \"zipcode\" is set, you can omit the zipcode to use your default.\n");
	...
	public void performService(AIMBuddy buddy, String query) {
		...
		if (imcommand.trim().equalsIgnoreCase("MOVIES")) {
			/*
			 * If there are three tokens then it's command, zip, theater. If
			 * there are only two then the second can either be theater or zip
			 * code (I check by length). If the second is a theater then I first
			 * use the zip from the theater search done to get that number. If
			 * there is no zip, because they went of memeory or used their
			 * default then I use the zipcode pref. If there are no tocken then
			 * I only use the zipcode pref not the zip saved if they did a
			 * theater search (bad? not sure).
			 */
			if (st.countTokens() == 2) {
			...
			if (zipcode != null && zipcode.length() == 5) {
				...
				if (zipPref == null) {
					// if the buddy didnt have a zippref set, set this one for
					// him
			...
			super.sendMessage(buddy, "<b>Legend:</b>\n"
...
class MovieTimesScraper extends ParserCallback {
	...
	/**
	 * Looks for link 'T0' for start of first theater. Then scrape text until T1
	 * link.
	 * 
	 * @param t
	 *            the tag
	...
	public void handleStartTag(HTML.Tag t, MutableAttributeSet a, int pos) {
		if ((t == HTML.Tag.A) && (a.getAttribute(HTML.Attribute.NAME) != null)
			...
			stage = 2;
				...
				&& (a.getAttribute(HTML.Attribute.NAME) != null)
				&& a.getAttribute(HTML.Attribute.NAME)
						.equals("T" + theaterNext)) {
			stage = 5;
		}
	}
	...
	public void handleText(char[] data, int pos) {
		...
		if (stage == 2 && !temp.equals("Add to My Favorite Theaters")) {
			...
			if (temp.toUpperCase().indexOf("BUY TICKETS") != -1) {
				 ...
				 * instead of one long string. Flag is set to create the one
				 * string.
				 */
				areLinkTimes = true;
			} else {
	...
	public ArrayList getTimes() {
		try {
			...
			logger.fine("Looking for movies for: " + zipcode + " using URL="
					+ url);
...
class MovieTheaterScraper extends ParserCallback {
	...
	public void handleStartTag(HTML.Tag t, MutableAttributeSet a, int pos) {
		...
		if ((t == HTML.Tag.A)
				&& (a.getAttribute(HTML.Attribute.HREF) != null)
				&& (((String) a.getAttribute(HTML.Attribute.HREF))
						.startsWith("#T")) && createList) {
			startA = true;
	...
	public String getTheaters() {
		try {
			URL url = new URL(urlPart + zipcode);
			logger.fine("Looking for theaters for: " + zipcode + " using URL="
					+ url);
			URLConnection conn = url.openConnection();
			BufferedReader br = new BufferedReader(new InputStreamReader(conn
					.getInputStream()));
