...
public class TicTacToeModule extends BotModule {
	...
	public String help() {
		...
		sb
				.append("<B>ttt <i>BUDDY</i></B> (starts a game with a buddy, if buddy is left blank, bot will play)\n");
		sb
				.append("<B>ttt <i>N</i></B> (tells tic tac toe what your move is, where N is the place to move)\n");
	...
	public void performService(AIMBuddy buddy, String query) {
		if (query.toLowerCase().startsWith("ttt")
				|| query.toLowerCase().startsWith("tictactoe")) {
			...
			if (tttg == null) {
				...
				if ((compname == null)
						|| compname.equalsIgnoreCase(bot.getUsername())) {
					...
					if (competitor != null) {
						...
						if (games.get(competitor.getName()) != null) {
							sendMessage(buddy, "Sorry, " + competitor.getName()
									+ " is in another tic tac toe game.");
						} else {
							// let the user know competitor has been asked.
							if (competitor.getName().equalsIgnoreCase(
									buddy.getName())) {
								sendMessage(buddy,
										"Sorry, no playing with yourself.");
							} else if (competitor.isOnline()) {
								tttg = new TicTacToeGame(you, competitor
										.getName());
								games.put(competitor.getName(), tttg);
								games.put(you, tttg);
								sendMessage(buddy, "Inviting " + compname
										+ " to play.");
								sendMessage(competitor,
										"Would you like to play Tic Tac Toe with "
												+ you + "? (ttt yes/ttt no)");
							} else {
								sendMessage(buddy, "Sorry, "
										+ competitor.getName()
										+ " isn't online.");
							}
						}
					} else {
						sendMessage(buddy, "Sorry, can't find " + compname
								+ " to play with.");
					}
				}
			} else {
				if (st.hasMoreTokens()) {
					...
					if (command.equals("end")) {
						...
						if ((tttg.state == tttg.REQUESTED)
								&& (tttg.getPlayerNumber(you) == 1)) {
							tttg.state = tttg.STARTED;
						...
						if ((tttg.state == tttg.REQUESTED)
								&& (tttg.getPlayerNumber(you) == 1)) {
							sendMessage(getBuddy(comp), you
									+ " said no to tic tac toe.");
							games.remove(comp);
							games.remove(you);
						} else {
							sendMessage(buddy,
						...
						if (tttg.state == tttg.STARTED) {
							try {
								...
								logger.severe("Error: " + command + ":"
										+ e.getMessage());
								sendMessage(buddy,
										"Unrecognized tic tac toe command: "
												+ command);
							}
						} else {
							sendMessage(buddy,
									"state of game not started.  sorry.");
						}
					}
				} else {
