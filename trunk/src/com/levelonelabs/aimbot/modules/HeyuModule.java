 ...
 * Linux/Unix and is written by Daniel B. Suthers. Heyu is available for free at
 * http://heyu.tanj.com. Daniel requests that NO mirrors of his Heyu source code
 * be created.
 * 
 ...
 * running and working).
 * 
 * Please see the Heyu FAQ at http://heyu.tanj.com for details about X10 and the
 * CM11A. The CM11A is available from http://www.x10.com and other sources for
 * under $50. Bundled deals that include X10 modules for the CM11A to control
 * can often be purchased at that price (a substantial discount). Please note:
 * The author is NOT trying to generate sales for X10, just simply stating how
 * you can get started using this module.
 * 
...
public class HeyuModule extends BotModule {
			...
			if (query.toLowerCase().startsWith("turn")) {
				try {
					Runtime.getRuntime().exec(
							heyuPath + " " + query.toLowerCase());
					logger.finest(buddy.getName() + " performed: "
							+ query.toLowerCase());
					...
					super.sendMessage(buddy, "Error processing command: "
							+ ioe.getMessage());
	...
	public HeyuModule(AIMBot bot) {
		...
		try {
			...
			logger.info("Error Reading HeyuModule.properties: "
					+ e.getMessage());
		}
	}
}